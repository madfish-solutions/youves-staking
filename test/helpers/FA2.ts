...
export class FA2 {
  ...
  async getBalance(user: string, tokenId: number = 0): Promise<BigNumber> {
    await this.updateStorage();
    const account: any = await this.storage.account_info.get(user);
    const balance = await account.balances.get(tokenId.toString());
    return balance !== undefined ? (balance as BigNumber) : new BigNumber(0);
  }
