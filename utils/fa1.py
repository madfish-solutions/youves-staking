...
class FA1GetBalance:
    """GetBalance object as per FA1.2 standard"""
    ...
    def get_type():
        ...
        Returns:
            sp.TRecord: single transfer type, layouted
        """
        return sp.TRecord(owner=sp.TAddress, callback=sp.TContract(sp.TNat)).layout(
            ("owner", "callback")
        )

    def make_one_request(owner, callback):
        """Creates a typed transfer item as per FA1.2 specification

        Args:
            owner (sp.address): address of the requested balance
            callback (sp.contract): callback contract

        Returns:
            GetBalance: transfer sp.record typed
        """
        return sp.set_type_expr(
            sp.record(owner=owner, callback=callback), FA1GetBalance.get_type()
        )
...
class FA12_core(sp.Contract, FA12_common):
    ...
    def getBalance(self, params):
        sp.set_type(params, sp.TRecord(owner = sp.TAddress, callback = sp.TContract(sp.TNat)).layout(("owner", "callback")))
        balance = sp.local("balance", sp.nat(0))
        with sp.if_(self.data.balances.contains(params.owner)):
            balance.value = self.data.balances[params.owner].balance
    
        sp.transfer(balance.value, sp.mutez(0), params.callback)
